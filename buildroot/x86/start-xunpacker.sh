...
PLUGIN_ARGS=`python3 ${SCRIPT_ROOT}/gen_args.py ${1} /tmp/vmlinux --size=32 --sex=1`
rm /tmp/vmlinux

echo qemu-system-i386 -M pc -m 256 -kernel ${IMAGE} -drive file=rootfs.ext2,if=virtio,format=raw -append \"rootwait root=/dev/vda console=tty1 console=ttyS0\" -net nic,model=virtio -net user -qmp unix:./qmp-sock,server,nowait -plugin ${QEMU_ROOT}/contrib/plugins/libxunpack32.so,${PLUGIN_ARGS},arg=uuid:${UUID} -D /tmp/qemu.log -d plugin -snapshot ${EXTRA_ARGS} &

exec qemu-system-i386 -M pc -m 256 -kernel ${IMAGE} -drive file=rootfs.ext2,if=virtio,format=raw -append "rootwait root=/dev/vda console=tty1 console=ttyS0"  -net nic,model=virtio -net user -qmp unix:./qmp-sock,server,nowait -plugin ${QEMU_ROOT}/contrib/plugins/libxunpack32.so,${PLUGIN_ARGS},arg=uuid:${UUID} -D /tmp/qemu.log -d plugin -snapshot ${EXTRA_ARGS} &
